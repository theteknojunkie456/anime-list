...
- **Read now (manga/manhwa/webtoons)** — a logged **reading** item gets a **Read now** button that opens **AsuraScans** (default) in the same in-app frame. AsuraScans has no deep-link (series URLs carry a random hash) so it opens the site's homepage to search. Swappable in Settings → *Watch & read sources* with an optional `{title}`/`{slug}` token.
