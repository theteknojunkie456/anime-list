...
- **Progress saving after watching** — we can't see inside the cross-origin player to auto-detect what you watched, so when you leave the player a quiet snackbar asks: *"Watched Ep 12? — Mark watched"*. One tap advances your progress (saved to localStorage + cloud sync); ignore it and nothing changes. Only offered when it would actually move you forward.
