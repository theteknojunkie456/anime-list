...
- **Password lock (end-to-end encryption)** — optionally set a password that encrypts your **entire list** with AES-GCM (key derived from your password via PBKDF2). The list is stored as ciphertext both on your device and in the cloud sync copy — genuinely unreadable without the password, even in browser devtools. On launch the app shows a lock screen until you enter it. Set/change/remove it in the backup panel. **If you forget the password, the data cannot be recovered.** (A friend viewing an encrypted list via `friends.html` must also be given the password.)
