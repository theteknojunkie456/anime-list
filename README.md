...
- **✦ AI panel** — the floating button at the bottom-right opens Suggest (what to watch next), Ask (questions about your list), and Rank (best viewing order for your plan list). Paste an API key into the panel — **free keys** from [Groq](https://console.groq.com) (`gsk_…`), [Google AI Studio](https://aistudio.google.com) (`AIza…`), or [OpenRouter](https://openrouter.ai) (`sk-or-…`), or a paid Claude key ([console.anthropic.com](https://console.anthropic.com), `sk-ant-…`). The app auto-detects the provider from the key; the key is stored only on that device. Everything else works without one
