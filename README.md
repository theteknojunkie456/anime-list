...
- **Watch now (anime, in-app)** — a logged **anime** (not manga, not live-action) gets a **Watch** button on its detail page that opens a streaming source *inside the app* — a full-screen player with a back button, deep-linked straight to that title. The base URL is a single constant (`STREAM_BASE` in `index.html`) because these sites rotate domains; when it dies, change one line. A ⤴ button opens the site's search if a title's slug doesn't resolve.
